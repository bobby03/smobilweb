...
class ModelController extends AdminController
{
	...
	public function actionList($name)
	{
		...
		if(get_class($model) === 'GeoProjInvest') 
                {
                    $toit = 'GeoProjectsGallery';
		}
                elseif(get_class($model) === 'GeoCursosCortosAdmin') 
		{
			$toit = 'GeoCoursesGallery';
                        $toitMaterials = 'GeoCursosMaterials';
                }
		elseif(get_class($model)==='GeoNoticia')
		{
			$toit = 'GeoNewGallery';
		}
		elseif(get_class($model)==='GeoUnidad')
		{
			$toit = 'GeoUnidadGallery';
		}
		elseif(get_class($model)==='GeoCategoria')
		{
			$toit = 'GeoGeotermia';
		}
		else 
		{
                    $toit = get_class($model);
                }
                
                if(get_class($model) === 'GeoDiplomados') 
		{
                        $toitMaterials = 'GeoDiplomadosMaterials';
                }
                if(get_class($model) === 'GeoEvents') 
		{
                        $toitMaterials = 'GeoEventsMaterials';
                }
                if(get_class($model) === 'GeoConvocatorias') 
		{
                        $toitMaterials = 'GeoConvocatoriasMaterials';
                }
		...
		$columns=array(
			...
			'customButtonUrl'=>$urlPrefix.'create",array("name"=>"'.$toit.'","pk"=>$data->primaryKey))',
			'materialsButtonUrl'=>$urlPrefix.'create",array("name"=>"'.$toitMaterials.'","pk"=>$data->primaryKey))',
			'customButtonOptions'=>array(
				'style'=>'display:none;',
			), 
                        'materialsButtonOptions'=>array(
				'style'=>'display:none;',
			), 
		...
		if ($this->module->getHideUpdateAdmin($model) === true) {
			$columns['updateButtonOptions'] = array(
				'style'=>'display:none;',
			);
		}
		if(get_class($model)==='GeoProjInvest' || get_class($model)==='GeoCursosCortosAdmin' || get_class($model)==='GeoNoticia' || get_class($model)==='GeoUnidad') {
			$columns['customButtonOptions'] = array(
				'style'=>'display:block;',
			);
//			$columns['viewButtonLabel'] = 'Galería';
			
		}
                if(get_class($model)==='GeoCursosCortosAdmin' || get_class($model)==='GeoDiplomados' || get_class($model)==='GeoEvents' || get_class($model)==='GeoConvocatorias' ) {
			$columns['materialsButtonOptions'] = array(
				'style'=>'display:block;',
			);
//			$columns['viewButtonLabel'] = 'Galería';
	...
	public function actionCreate($name)
	{
		...
		if (Yii::app()->request->isPostRequest && isset($_POST[$name])) {
			...
			$model->extraPhpBeforeSaveValidate($model,(isset($_POST)?$_POST:''),$paths,$this->module);
			
			if ($model->save()) {
				$model->extraPhpAfterSaveValidate($model,(isset($_POST)?$_POST:''),$paths,$this->module);
				Yii::app()->user->setFlash('success',Yii::t('YcmModule.ycm','Cambios guardados.'));
	...
	protected function redirectUser($name,$pk)
	{
		if (isset($_POST['_save'])) {
			...
			Yii::app()->user->setFlash('success',Yii::t('YcmModule.ycm','Cambios Guardados, Puedes agregar otro.'));
			if($name === 'GeoProjectsGallery') {
				$mod = GeoProjectsGallery::model()->findByPk($pk);
				foreach($mod as $data) {
					$pk = $data->id_projects;
				}
			}
			$this->redirect($this->createUrl('model/create',array('name'=>$name,'pk'=>$pk)));
