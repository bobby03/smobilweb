			...
			case 'chosen':
				$options=array(
					'empty'=>Yii::t('YcmModule.ycm',
						'Selecciona {name}',
//						array('{name}'=>mb_strtolower($model->getAttributeLabel($attribute), 'UTF-8'))
						array('{name}'=>strtolower($model->getAttributeLabel($attribute)))
					),
...
//						array('{name}'=>mb_strtolower($model->getAttributeLabel($attribute), 'UTF-8'))
			...
			case 'file':
				...
				if (!$model->isNewRecord && !empty($model->$attribute)) {
					...
					$this->controller->widget('bootstrap.widgets.TbButton',array(
						'label'=>Yii::t('YcmModule.ycm','Descargar'),
						'type'=>'',
			...
			case 'image':
				...
				if (!$model->isNewRecord && !empty($model->$attribute)) {
					...
					echo '<p>';
					$this->controller->widget('bootstrap.widgets.TbButton',array(
						'label'=>Yii::t('YcmModule.ycm','Vista previa'),
						'type'=>'',
						'htmlOptions'=>array(
							'data-toggle'=>'modal',
	...
	public function getAttributeWidget($model,$attribute)
	{
		if ($this->attributesWidgets!==null) {
			if (isset($this->attributesWidgets->$attribute)) {
				...
				$dbType=(isset($model->tableSchema->columns[$attribute]) && 
                                            $model->tableSchema->columns[$attribute]->dbType?
                                                $model->tableSchema->columns[$attribute]->dbType:"");
				if ($dbType=='text') {
					return 'wysiwyg';
				} else {
	...
	public function getHideUpdateAdmin($model)
	{
		if (is_string($model)) {
			$model=new $model;
		}
		if (isset($model->hideUpdateActionAdmin)) {
			return $model->hideUpdateActionAdmin;
		} else {
			return false;
		}
	}        
        public function getHideUpdateIndex($model){
            if (is_string($model)) {
			$model=new $model;
		}
		if (isset($model->hideUpdateIndex)) {
			return $model->hideUpdateIndex;
		} else {
			return true;
		}
        }
